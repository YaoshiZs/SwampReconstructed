use crate::compilers::analyzer::ast::node::ExprNode;
...
pub fn resolve(tokens: &mut TokenVec, id_paren: Paren) -> Result<Vec<ExprNode>, ()> {
    // examples:
    // 1, 2)
    // a, 1)
    // 1, [2, 3])
    ...
    fn element_resolve(
        ...
        elements: &mut Vec<ExprNode>,
    ) -> Result<(), ()> {
        if sub_tokens.len() > 0 {
            let element = expr_resolver::resolve(sub_tokens)?;
            sub_tokens.clear();
    ...
    let mut elements = Vec::<ExprNode>::new();
    ...
    loop {
        ...
        let is_id_paren = current == Token::Paren(id_paren);
